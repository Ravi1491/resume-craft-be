...
const formatAjvErrors = (errors: ErrorObject[]): ValidationError[] => {
    return errors.map((error) => ({
        field: error.instancePath ? error.instancePath.replace(/^\//, '') : error.schemaPath,
...
// Custom AJV keyword for validating date and time ranges together
ajv.addKeyword({
    keyword: 'validateDateTimeRange',
    type: 'object',
    validate: function validate(
        ...
        if (!data.startTime || !data.endTime || !data.startDate || !data.endDate) return true;
